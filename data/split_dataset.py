...
class Splitter(ABC):
    ...
    @abstractmethod
    def split(self, clients: int, local_data_amount: int, dataset: VisionDataset):
...
class DirichletUnbalanced(Splitter):
    ...
    def split(
        self, clients: int, local_data_amount: int, dataset: VisionDataset) -> dict:
        ...
        max_oversample = samples.sum(axis=0).max()
        if max_oversample * local_data_amount < idx.shape[1]:
                max_oversample = 1
...
if __name__ == "__main__":
    ...
    from make_dataset import get_cifar10, DATA_PATH
    ...
    plt.savefig(f"./splits({alpha}).pdf")
