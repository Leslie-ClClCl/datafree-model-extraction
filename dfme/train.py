...
def train(args, teacher, student, generator, device, optimizer, epoch):
    ...
    for i in range(args.epoch_itrs):
        ...
        for _ in range(args.g_iter):
            ...
            # Compute Real Loss
            pred_teacher = teacher(fake)
            pred_student = student(fake)
            if args.loss == "l1":
                loss_fn = F.l1_loss
                if args.no_logits:
                    pred_teacher_no_logits = F.log_softmax(pred_teacher, dim=1)
                    if args.logit_correction == 'min':
                        pred_teacher = pred_teacher_no_logits - pred_teacher_no_logits.min(dim=1).values.view(-1, 1)
                    elif args.logit_correction == 'mean':
                        pred_teacher = pred_teacher_no_logits - pred_teacher_no_logits.mean(dim=1).view(-1, 1)
                    else:
                        pred_teacher = pred_teacher_no_logits

            elif args.loss == "kl":
                loss_fn = F.kl_div
                pred_student = F.log_softmax(pred_student, dim=1)
                pred_teacher = F.softmax(pred_teacher, dim=1)
            else:
                raise ValueError(args.loss)

            loss_G = -loss_fn(pred_student, pred_teacher, reduction='mean')
            loss_G.backward()
            optimizer_G.step()
...
def main():
    ...
    with open('log/ -%s.csv' % (args.dataset), 'a') as f:
        writer = csv.writer(f)
